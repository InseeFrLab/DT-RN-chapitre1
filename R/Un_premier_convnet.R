# import des librairies nécessaires pour le chargement des images
library("keras")
library("tensorflow")
...
x_train = array_reshape(x_train,dim=c(dim(x_train)[1],32,32,1))
x_test = array_reshape(x_test,dim=c(dim(x_test)[1],32,32,1))
...
get_heatmap <- function(no_input=1,no_output=2){
  ...
  c(pooled_grads_value, conv_layer_output_value) %<-% iterate(list(input_image_data))
...
heatmap <- get_heatmap(no_input=i,no_output=2)   
...
for (i in 1:25) { 
  ...
  heatmap <- get_heatmap(no_input=i,no_output=predicted_label+1)
  ...
  alpha <- floor(seq(255,255,length=ncol(pal)))
  pal_col <- rgb(t(pal),alpha=alpha,maxColorValue = 255)
  #write_heatmap(matrix(heatmap,c(32,32)),"original_overlay.png",width = 14,height = 14,bg = NA,col=rev(palette))
